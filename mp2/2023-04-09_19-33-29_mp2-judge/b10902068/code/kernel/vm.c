...
void format(int i, uint64 pte, uint64 va, uint64 pa, pte_t * PTE){
  ...
  if (*PTE & PTE_S)
    printf("+-- %d: pte=%p va=%p blockno=%p", i, pte, va , pa);
  else 
...
void vmprint(pagetable_t pagetable) {
  ...
  for (uint64 i = 0; i < 512; i++){
    uint64 va = pre_va | (i << (12 + level * 9));
    pte_t * pte = &pagetable[PX(level, va)];

    if (*pte & PTE_V || *pte & PTE_S) max_i = i;
  }
  ...
  for (uint64 i = 0; i < 512; i++){
    ...
    if (*pte & PTE_V || *pte & PTE_S){
	  ...
	  cont[level] = (max_i == i)? 0 : 1;
	  format(i, (uint64)(&pagetable[i]), va ,PTE2PA(*pte), pte);

          level --;
	  uint64 tmp_va = pre_va;
	  pre_va = va;
...
int madvise(uint64 base, uint64 len, int advice) {
  ...
  /* TODO */
  if (base < 0 || len < 0 || base + len > myproc() -> sz)
    return -1;
  ...
  for(;;){
	...
	begin_op();
        ...
        mappages(pagetable, va, PGSIZE, (uint64)pa, perm);
      }
    }
