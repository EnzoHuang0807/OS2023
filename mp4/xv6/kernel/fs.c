...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < MAXFILE - NDIRECT - NINDIRECT){
    int index0 = bn / (256 * 256);
    int index1 = (bn / 256) % 256;
    int index2 = bn % 256;

    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT + 1 + index0]) == 0)
      ip->addrs[NDIRECT + 1 + index0] = addr = balloc(ip->dev);
